...
2. For each visible order, `getJobByOrderId()` calls `fetchJobsSince()` — an OData query filtered by `CreationTime gt <card date>` and `contains(OutputArguments, '<orderUid>')`. The lookback date comes from `getCardDate()` (parses the card's `.date p` `MM/DD/YYYY` text, defaulting to 30 days).
3. Candidate jobs are fetched in detail (batched, 10 at a time via `Promise.allSettled`), their `OutputArguments` JSON is parsed, and a job matches when `output.out_OrderUid === orderId`.
4. Results stream to the popup via `SCAN_RESULTS` messages and are persisted (see storage below).
